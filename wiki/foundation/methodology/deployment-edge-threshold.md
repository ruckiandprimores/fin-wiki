...
**Calibration sources (added 2026-07-12).** Two ingested sources now anchor the working numbers above: [[foundation/sources/talos-crypto-market-impact]] — the first large-sample crypto-specific impact model (50k+ parent orders, top-60 spot + perps, Jun 2024–Jul 2025; spread + temporary-impact + time-risk decomposition; >26% of institutional trades land in the 0–5bp slippage range, and plain square-root plug-ins *underestimate* small-order cost by ~4bp because spread cost is size-independent) — and [[foundation/sources/maitrier-metaorder-reconstruction]] — a fully public-data method for measuring metaorder impact in-house on Binance/Bybit tapes (aggressor flags suffice; no proprietary data needed), i.e. the route to a venue-specific impact curve instead of an equity-calibrated constant. **Honest gap (2026-07 research pass): no academic study of paper-fill→live-fill degradation exists** — the Talos numbers are institutional-flow-specific and the Maitrier method is unreplicated in crypto; the daily observation-trail loop (entry-vs-outcome records) remains the primary live-vs-backtest calibration research.
