...
- **Liquidation-magnet trades**: when liquidation cluster density is high above current price, price often *gravitates* toward the cluster (predictable directional bias). Trade: position long below the cluster anticipating the magnet effect. **Now drafted as [[trading/mechanisms/liquidation-heatmap-signal]]** (2026-07-12; structured hypothesis, research-grade only).
...
- [[foundation/sources/hyblock-coinglass-liquidation-heatmaps]] — **added 2026-07-12**: how the liquidation maps this family leans on are actually constructed — both Hyblock and Coinglass are MODELED estimates (leverage-tier projection / opaque OI model), not exchange engine output; prerequisite reading before treating `liq_cluster_density` as ground truth
- [[trading/mechanisms/liquidation-heatmap-signal]] — **added 2026-07-12**: promotes this page's "liquidation-magnet trades" candidate mechanism to its own page — structured magnet hypothesis (research-grade only; no academic test exists) + in-house validation experiment via Hyblock API
